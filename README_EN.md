...
## Gateway & Multi-Harness Compatibility

### 1. OpenAI-Compatible Gateway & Proxy Best Practices
When using third-party proxies (e.g., OneAPI, NewAPI, or custom gateways) to proxy Gemini models:
- **`anyOf` Union Schema Compatibility**: Some coding harnesses (e.g., OMP's built-in `yield` tool) define union parameter types (such as `type: ["string", "array"]` with `items: { type: "string" }`). Certain OpenAI-to-Gemini translation proxies incorrectly place the `items` property on non-array nodes, triggering HTTP 400 Bad Request errors on the subagent's initial request.
- **Recommendation**: Ensure your proxy engine correctly parses `anyOf` and union types when converting OpenAI function definitions into Google Generative AI tools (e.g., using `router-for-me/CLIProxyAPI` or direct Google API endpoints), or configure passthrough endpoints.

### 2. Harness-Specific Requirements
- **Pi (`pi`)**: Requires the `pi-subagents` extension (`pi install npm:pi-subagents`). Coordinators spawn workers via the native `subagent` tool.
- **OpenCode (`opencode`)**: Requires `subagent_depth >= 3` in `opencode.jsonc`. Coordinators spawn workers using OpenCode's schema `{ subagent_type, prompt, description }`.
- **OMP (`omp`)**: Requires `task.maxRecursionDepth >= 3` and recommended `task.softRequestBudget >= 80` in `~/.omp/agent/config.yml`.
...
├── README.md                      # Primary documentation (Simplified Chinese)
