...
/**
 * Strips outer JSON wrappers and markdown code fences from model outputs.
 * @param {string} text
 * @returns {string}
 */
export function cleanReportOutput(text) {
  if (!text) return "";
...
/**
 * Parses and validates skepticism protocol sections in worker reports.
 * @param {string} text
 * @returns {{
 *   raw: string;
 *   cleaned: string;
 ...
 * }}
 */
export function parseSkepticismReport(text) {
